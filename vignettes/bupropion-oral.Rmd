...
Bupropion is an antidepressant of the aminoketone class and indicated for the treatment of major depressive disorder [@Fava2005]. Wellbutrin® was first marketed as an immediate release (IR) product in 1980 in 75 and 100 mg tablets, 3-times daily administration [@Connarn2017]. In 1996, a sustained release (SR) formulation was made available for twice daily administration as 100 and 150 mg tablets, and, in 2003 an extended formulation (ER) was developed for once-daily administration as 150 and 200 mg tablets [@Dhillon2008].    This case study evaluates virtually the potential for BE of the 150 mg sustained release (SR) tablet, which, for the purposes of this case study, is considered to be the R formulation, and the 150 mg extended release (ER) tablet, considered here to be the T formulation.
  
  
## Oral application case study data sets
  
In this example, data set D1 consists of bupropion plasma concentration versus time profiles reported in Connarn et al. [@Connarn2017] for 30 adults who received a 150 mg dose via the SR tablet formulation (R).  Data set D2 consists of the weights and heights of  each individual in data set D1. The data were generated within the scope of FDA-funded contract HHSF223201310164C and fully reported by Connarn et al. [@Connarn2017]. 
...
The models differ only in the tablet dissolution profiles for the SR and ER formulations, both of which were reported in Marok et al. [@Marok2021] and are characterized by Weibull kinetics.   These profiles are shown in Figure 6A. In the model, the Weibull kinetics express the time-dependent undissolved fraction $F(t)$ of the tablet as:
...
Note that `library(ospsuite.openVBE)` also loads `ospsuite` package which allows to directly access the namespace of the latter package.

> `ospDimensions` and `ospUnits` list all the dimensions and units available in the OSP Suite. 
> These variables are exported by the `ospsuite` package. Loading `ospsuite.openVBE` then allows to directly access them.
...
inferenceParameters <- list(Theta1,Theta2,LiverIntestinalCL)
```

Learns the posterior distributions of `Theta1`, `Theta2`, and `LiverIntestinalCL` IIV parameters
