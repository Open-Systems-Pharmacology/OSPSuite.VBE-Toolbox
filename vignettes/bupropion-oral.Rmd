...
Note that `library(ospsuite.VBEToolbox)` also loads `ospsuite` package which allows to directly access the namespace of the latter package.

> `ospDimensions` and `ospUnits` list all the dimensions and units available in the OSP Suite. 
> These variables are exported by the `ospsuite` package. Loading `ospsuite.VBEToolbox` then allows to directly access them.
